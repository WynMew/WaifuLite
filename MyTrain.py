...
for i in ibar:
    ...
    for index, batch in enumerate(img_data):
        ...
        if (counter + 1) % 500 == 0:
            torch.save(model.state_dict(), './SavedModel/CARN_model_checkpoint.pt')
            torch.save(optimizer.state_dict(), './SavedModel/CARN_adam_checkpoint.pt')
            torch.save(train_loss, './SavedModel/train_loss.pt')
            torch.save(train_ssim, "./SavedModel/train_ssim.pt")
            torch.save(train_psnr, './SavedModel/train_psnr.pt')
            torch.save(scheduler.last_batch_iteration, "./SavedModel/CARN_scheduler_last_iter.pt")
    ...
    with torch.no_grad():
        ...
        torch.save(test_loss, './SavedModel/test_loss.pt')
        torch.save(test_ssim, './SavedModel/test_ssim.pt')
        torch.save(test_psnr, './SavedModel/test_psnr.pt')
